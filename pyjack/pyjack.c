...
typedef struct {
    ...
    float         input_meters[PYJACK_MAX_PORTS];
...
void pyjack_init(pyjack_client_t * client) {
    ...
    // Init buffers to          printf("outputing\n");null...
...
//            float _omega = 9.72f / 41000;
            //float period = float(1024/41000);
            

//            float z1 = 0;
//            float z2 = 0;

            float *sample = jack_port_get_buffer(client->input_ports[i], n);

            int s_remaining = client->buffer_size;
            float *peak_sample = &(client->input_meters[i]);

            while(s_remaining--) {
                if(fabs(*sample) > *peak_sample) peak_sample = sample;
//                float sample_squared = *sample * *sample;
//                z1 += _omega * (sample_squared - z1);
//                z2 += _omega *(z1 - z2);                

                sample++;
            }

            client->input_meters[i] = fabs(*peak_sample);
//            float rms = 20 * log10f(sqrtf(2 * z2));
//            float peak = 20 * log10f(fabs(*peak_sample));            
...
//            printf("RMS: %f PEAK: %f\n", rms, peak);
    ...
    if (client->num_outputs) {
        ...
        for(i = 0; i < client->num_outputs; i++) {
            ...
            float *sample = jack_port_get_buffer(client->output_ports[i], client->buffer_size);

            int s_remaining = client->buffer_size;
            float *peak_sample = sample;
            ...
            float rms = sqrtf(2 * z2);
            float peak = fabs(*peak_sample);
            ...
            //printf("\n");
            //printf("OUT RMS: %1.15f PEAK: %1.15f\n", rms, peak);
        }
    }
...
static PyObject* attach(PyObject* self, PyObject* args)
{
    ...
    // TODO: allow option for JackNoStartServer
    client->pjc = jack_client_open(cname, /*JackNoStartServer*/ 0, &status);
    if(client->pjc == NULL) {
        //TODO check status
...
//     }
    ...
    for (i=0;i<client->num_inputs;i++) {
        ...
        for(;mi<client->num_inputs;mi++) {
            client->input_meters[i] = client->input_meters[i+1];
        }
...
//     }
    ...
    if(flags & JackPortIsInput) {
        ...
        client->input_meters[client->num_inputs] = 0;
        client->num_inputs++;
    }
...
static PyObject* get_ports(PyObject* self, PyObject* args)
{
    ...
    if(jplist != NULL) {
        while(jplist[i] != NULL) {
            PyObject* pname = Py_BuildValue("s", jplist[i]);
...
static PyObject* get_input_meters(PyObject* self, PyObject* args)
{
    pyjack_client_t * client = self_or_global_client(self);

    PyObject* plist = PyList_New(client->num_inputs);
    int i;
    for(i=0; i<client->num_inputs; i++) {
        float peak_dB  = 20 * log10f(client->input_meters[i]);
        PyList_SetItem(plist, i, Py_BuildValue("f", peak_dB));
        client->input_meters[i] = 0.0;
    }
...
static PyMethodDef pyjack_methods[] = {
  ...
  {"get_input_meters",   get_input_meters,        METH_VARARGS, "get_input_meters():\n Get a list of all dB values for client inputs"},
